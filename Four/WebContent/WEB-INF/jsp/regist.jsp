...
<script>
	...
	function checkUsername() {
		...
		// 3.打开连接
		xmlHttp.open("POST", "${pageContext.request.contextPath}/ajax?username="
				+ encodeURI(username), false);
		// 4.发送
		xmlHttp.send();
	}
	/* 	function checkUsername() {
	 ...
	 xhr.open("GET",
	 "${pageContext.request.contextPath}/ajaxvalidator?username="
	 + username, true);
	 xhr.send(null);
	 alert("已填写用户名");
	 } */
	...
	function checkPhone() {
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4 && xhr.status == 200) {
				document.getElementById("phonespan").innerHTML = xhr.responseText;
			}
		}
		xhr.se
		xhr.open("POST", "${pageContext.request.contextPath}/ajaxphone?phone="
				+ encodeURI(phone), true);
		xhr.send();
	}
	function checkAddress(){
		$.ajax({
			type:"POST",
		    url:"${pageContext.request.contextPath}/ajaxaddress", 
			data:"address=wyxy", 
			...
			<div class="wrap">
				<div class="main clearfix">
					<div class="title">
						...
						<c:if test="${registerror==null}">USER REGIST</c:if>
					</div>
					<div>
						<s:actionerror />
					</div>
					<form id="registerForm"
						action="${ pageContext.request.contextPath }/registing"
							...
							<tbody>
								...
								<tr>
									...
									<td><input type="text" name="name" class="text"
										maxlength="200" /> <span><c:forEach items="${ validationError}" var="ve">
										<c:out value="${ve.defaultMessage}"/>
										</c:forEach></span></td>
								</tr>
								...
								<tr>
									...
									<td><input type="text" name="phone" id="phone"class="text" 
										onblur="checkPhone()" /><span id="phonespan"></span></td>
								</tr>

								<tr>
									<th>地址:</th>
									<td><input id="address" type="text" name="address" class="text" 
										maxlength="200" onblur="checkAddress()"/> <span id="addressspan"></span></td>
								</tr>
								<tr>
									<%-- <th>

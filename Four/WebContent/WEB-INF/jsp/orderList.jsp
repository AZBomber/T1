...
<body>
	...
	<div class="container cart">
		...
		<div class="span24">
			...
			<table>
				<tbody>
					<c:forEach items="myorder" var="my">
						<tr>
						...
						<c:forEach items="${myorder }" var="my">
							...
							<tr>
								...
								<td width="60"><c:forEach items="${my.orderItems}" var="mo">
										<img
											src="${ pageContext.request.contextPath }/${mo.product.image}" />
									</c:forEach></td>



								<td><c:forEach items="${my.orderItems}" var="mo">${mo.product.pname }:*${mo.count }小计:${mo.subtotal }</c:forEach></td>

								<%-- <td>${mo.subtotal }</td>
								<td class="quantity" width="60">${mo.count }</td>	 --%>
								<td width="140"><span class="subtotal">￥${my.total}
								</span></td>
								<td><c:if test="${my.state==1}">
								未支付
								</c:if> <c:if test="${my.state!=1}">
								已支付
								</c:if></td>
							</tr>
							</a>
